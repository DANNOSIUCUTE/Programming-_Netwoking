...
def send_chunk_part_sliding_window(sock, client_addr, filename, offset, size, part_id):
    ...
    SAFE_UDP_SIZE = 1200     # Kích thước tối đa gói UDP an toàn (điều chỉnh theo môi trường)
...
def handle_chunk(filename, offset, size, part_id, client_addr):
    """
    Hàm này chạy trên một thread riêng, tạo socket mới và gửi chunk qua socket đó.
    """
    # Tạo socket UDP mới và bind vào cổng ngẫu nhiên
    sock_chunk = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    sock_chunk.bind(("0.0.0.0", 0))
...
def main():
    ...
    while True:
        try:
            ...
            # Xử lý yêu cầu "LIST"
            if message == "LIST":
                send_file_list(sock_main, client_addr)
            ...
            # Xử lý yêu cầu "CHUNK <filename> <offset> <size> <part_id>"
                ...
                t = threading.Thread(
                    target=handle_chunk,
                    args=(filename, offset, size, part_id, client_addr)
                )
                t.start()
